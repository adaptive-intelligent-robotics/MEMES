...
from core.emitters.nses_emitter import NSESConfig, NSESEmitter
...
class ExperimentConfig:
    ...
    adaptive_fitness_weight: bool
    adaptive_generations_period: int
    adaptive_amount: float


@hydra.main(config_path="configs", config_name="ns_es")
def train(config: ExperimentConfig) -> None:
    ...
    if config.num_iterations > 0:
        ...
        num_iterations = (
            config.num_evaluations // (config.batch_size * config.sample_number) + 1
        )
    ...
    # Define emitter
    nses_emitter_config = NSESConfig(
        ...
        adaptive_fitness_weight=config.adaptive_fitness_weight,
    )

    nses_emitter = NSESEmitter(
        config=nses_emitter_config,
