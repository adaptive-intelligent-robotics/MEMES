...
class QualityPGEmitter(Emitter):
    ...
    def init(
        ...
        emitter_state = QualityPGEmitterState(
            ...
            parents=jax.tree_util.tree_map(lambda x: x[:-1], init_genotypes),
            parents_descriptors=jnp.zeros(
                (self._config.env_batch_size - 1, self._env.behavior_descriptor_length)
            ),
            parents_distance=0,
        )
    ...
    def emit(
        ...
        offsprings_pg = self.emit_pg(emitter_state, emitter_state.parents)
    ...
    def state_update(
        ...
        # Sample parents and update parents_distance metrics
            ...
            jnp.sqrt(
                jnp.sum(
                    jnp.square(
                        ...
                        - descriptors[:mutation_pg_batch_size]
                    ),
                    axis=1,
                )
            ),
        ...
        parents, parents_descriptors, _, random_key = repertoire.sample_full(
            emitter_state.random_key, mutation_pg_batch_size
        )
