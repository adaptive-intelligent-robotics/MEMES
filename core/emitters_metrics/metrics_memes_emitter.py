...
from core.emitters_metrics.metrics_memes_fix_reset_emitter import (
...
class MEMESEmitterState(MEMESFixResetEmitterState):
    ...
    Args:
        ...
        emitter_gen_reset_count: number of generations since last reset
        emitter_max_gen_reset_count: max number of generations between reset
        emitter_mean_gen_reset_count: mean number of generations between reset
        emitter_n_reset_count: number of resets
    ...
    emitter_gen_reset_count: jnp.ndarray
    emitter_max_gen_reset_count: jnp.ndarray
    emitter_mean_gen_reset_count: jnp.ndarray
    emitter_n_reset_count: jnp.ndarray
    random_key: RNGKey
...
class MEMESEmitter(MEMESFixResetEmitter):
    ...
    def init(
        ...
        return (
            MEMESEmitterState(
                ...
                emitter_gen_reset_count=jnp.zeros(self._batch_size),
                emitter_max_gen_reset_count=jnp.zeros(self._batch_size),
                emitter_mean_gen_reset_count=jnp.zeros(self._batch_size),
                emitter_n_reset_count=jnp.zeros(self._batch_size),
    ...
    def _sample_parents(
        ...
        added: jnp.ndarray,
        batch_size: int,
    ) -> Tuple[MEMESEmitterState, Genotype, Descriptor, optax.OptState, RNGKey]:
        """Sample new parents."""
        ...
        (
            ...
            parents_fitnesses,
            random_key,
        ) = repertoire.sample_full(random_key, batch_size)
        ...
        parents_descriptors = jnp.where(
            jnp.reshape(
                jnp.repeat(
                    reset_emitter,
                    self._num_descriptors,
                    total_repeat_length=self._num_descriptors * batch_size,
                ),
                parents_descriptors.shape,
            ),
            parents_descriptors,
            descriptors,
        )
        parents_fitnesses = jnp.where(reset_emitter, parents_fitnesses, fitnesses)
        optimizer_states = jax.tree_util.tree_map(
        ...
        # Update emitter reset metrics
        emitter_n_reset_count = jnp.where(
            reset_emitter,
            emitter_state.emitter_n_reset_count + 1,
            emitter_state.emitter_n_reset_count,
        )
        emitter_gen_reset_count = jnp.where(
            reset_emitter, 0, emitter_state.emitter_gen_reset_count + 1
        )
        emitter_max_gen_reset_count = jnp.maximum(
            emitter_state.emitter_max_gen_reset_count, emitter_gen_reset_count
        )
        emitter_mean_gen_reset_count = jnp.where(
            reset_emitter,
            1
            / emitter_n_reset_count
            * (
                emitter_state.emitter_n_reset_count
                * emitter_state.emitter_max_gen_reset_count
                + emitter_gen_reset_count
            ),
            emitter_state.emitter_mean_gen_reset_count,
        )
        emitter_state = emitter_state.replace(
            emitter_adaptive_reset=emitter_adaptive_reset,
            emitter_gen_reset_count=emitter_gen_reset_count,
            emitter_max_gen_reset_count=emitter_max_gen_reset_count,
            emitter_mean_gen_reset_count=emitter_mean_gen_reset_count,
            emitter_n_reset_count=emitter_n_reset_count,
        )
