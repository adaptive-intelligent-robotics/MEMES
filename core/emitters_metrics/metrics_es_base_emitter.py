...
class ESEmitter(Emitter):
    ...
    def _sample_parents(
        ...
        added: jnp.ndarray,
        batch_size: int,
    ) -> Tuple[ESEmitterState, Genotype, Descriptor, optax.OptState, RNGKey]:
        """Sample new parents."""
    ...
    def state_update(
        ...
        """
        Update the generation count and usage metrics from current call.
        Generate the gradient offsprings for the next emitter call.
        """
        assert emitter_state is not None, "\n!!!ERROR!!! No emitter state."

        # Check if offspring have been added to repertoire
        exploit_usage = added_repertoire(genotypes, descriptors, repertoire)
