...
class MultiEmitter(Emitter):
    ...
    def init(
        ...
        # init usage
        usage = jnp.zeros(len(emitter_states))

        return (
            MultiEmitterState(emitter_states=tuple(emitter_states), usage=usage),
            random_key,
        )
    ...
    def state_update(
        ...
        for emitter, sub_emitter_state, index_start, index_end in zip(
            ...
            usage = usage.at[idx].set(
                jnp.sum(added_repertoire(sub_gen, sub_desc, repertoire))
            )
            idx += 1
        ...
        return MultiEmitterState(tuple(emitter_states), usage)
